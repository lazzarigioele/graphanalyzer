...
`graphanalyzer.py` is Python 3.7 script developed locally on a 2019 Intel MacBook Pro machine, but it should run on every UNIX system. It mainly relies on **networkx** (https://networkx.org) to work with graphs and **pandas** (https://pandas.pydata.org) to work with tables. The script also uses some other external libraries and we suggest to run it into a dedicated conda environment created as follows.
