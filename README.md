...
* [Main algorithm](https://github.com/lazzarigioele/graphanalyzer#description-of-the-main-algorithm) description of what's under the hood
...
![screenshot-v1.2.2.png](assets/screenshot-v1.2.2.png)
...
`graphanalyzer.py` is a Python script and should run on every UNIX system. It relies on some external libraries like [networkx](https://networkx.org), [pandas](https://pandas.pydata.org), [hvPlot](https://hvplot.holoviz.org), [openpyxl](https://openpyxl.readthedocs.io/) and [pygraphviz](https://pygraphviz.github.io). We suggest to run it into a dedicated conda environment. For both MacOSX and Linux we provide a precise definition of the minimum conda environment required for running `graphanalyzer.py`. You can find it at `./env_macosx.yml` and `./env_linux.yml`. Given this definition, you can easily create the right conda environment using the following command `conda env create -f ./env_macosx.yml`. 
...
In this section, we will examine some example vOTUs in order to better understand how `graphanalyzer.py` works. All vOTUs discussed in this section are contained here in `./testinput/`, so every user can test the script with the same data and obtain the same results.  
