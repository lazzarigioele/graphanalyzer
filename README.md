...
`--skipdiff` (of `-k`) defines whether to skip differentiation between 'A' Level and 'F' Level (default: `False`). If set to `True` both 'A' and 'F' Levels will appear under a common 'AF' Level. If set to `True`, computation time could be reduced by 10 minutes or more, depending on the complexity of your data. 
